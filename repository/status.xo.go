...
func (s *Status) Insert(db XODB) error {
	...
	res, err := db.Exec(sqlstr, s.Code, s.Name, s.CreateAt, s.UpdateAt)
	if err != nil {
		return err
	}
...
func (s *Status) Update(db XODB) error {
	...
	_, err = db.Exec(sqlstr, s.Code, s.Name, s.CreateAt, s.UpdateAt, s.ID)
	return err
}
...
func StatusByID(db XODB, id sql.NullInt64) (*Status, error) {
	var err error
